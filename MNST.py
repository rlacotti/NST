...
def load_data(content_dir: str, style1_dir: str, style2_dir: str, transform=None):
    ...
    style2_dataset = NSTDataset(style2_dir, transform=transforms)
    ...
    style2_loader = DataLoader(style2_dataset, batch_size=len(style2_dataset))
    ...
    style2_images = next(iter(style2_loader))

    return content_images, style1_images, style2_images
...
def main():
    ...
    content_images, style1_images, style2_images = load_data(contentDir, style1Dir, style2Dir)
    ...
    style1_img = style1_images[7].to(device)
    style2_img = style2_images[19].to(device)
    ...
    for step in range(steps):
        ...
        style2_features = model(style2_img)

        # Initialize both style and content loss
        style1_loss = style2_loss = content_loss = 0

        for gen_feature, con_feature, sty1_feature, sty2_feature in zip(generated_features, content_features, style1_features, style2_features):
            ...
            # Gram matrix for style1 image --> A1 = S1 * S1^T
            A1 = sty1_feature.view(channel, height*width).mm(
                sty1_feature.view(channel, height*width).t()
            )

            # Gram matrix for style2 image --> A2 = S2 * S2^T
            A2 = sty2_feature.view(channel, height*width).mm(
                sty2_feature.view(channel, height*width).t()
            )

            # Style loss Formula
            style1_loss += torch.mean((G - A1)**2) / (channel * height * width)
            style2_loss += torch.mean((G - A2)**2) / (channel * height * width)

        # Total loss Formula
        total_loss = alpha*content_loss + beta1*style1_loss + beta2*style2_loss
        ...
        if step % 500 == 0:
            ...
            file_path = os.path.join(genDir, 'MNSTsamurai2.png')
    ...
    # Plot original Style1 image
    plt.figure()
    imshow(style1_img)

    # Plot original Style2 image
    plt.figure()
    imshow(style2_img)
